...
void score_each_swap(Superball *&s, Disjoint_Set &d, int &num1, int &num2, multimap<int, vector<int>, greater<int> > &all_swap_scoring) {
...
///////////////
    ...
    int total_size_sets_in_scoring_cells = 0; 
    ...
    for (djs_sizes_descending_order_it = djs_sizes_descending_order.begin(); djs_sizes_descending_order_it != djs_sizes_descending_order.end(); djs_sizes_descending_order_it++) {
        ...
        total_size_sets_in_scoring_cells+= djs_sizes_descending_order_it->first;
    }
    djs_sizes_descending_order_it = djs_sizes_descending_order.begin();
    int greatest_size = djs_sizes_descending_order_it->first;

    cout << "num1 " << num1 << "num2 " << num2 << endl; 
    cout << "total_size_sets_in_scoring_cells " << total_size_sets_in_scoring_cells << endl;
...
void enumerate_pairs(Superball *&s, Disjoint_Set &d, multimap<int, vector <int>, greater<int> > &all_scores) {
    Disjoint_Set d2;
    ...
    for (int i = start; i < s->r*s->c; i++) { // replace with... s->r*s->c
        for (int j = (start+1); j < s->r*s->c; j++) {
            ...
            if (isalpha((char)s->board[i]) && isalpha((char)s->board[j]) && ((char)s->board[i] != (char)s->board[j])) {
              ...
              //print_board(s);
              create_sets(s, d2); 
              score_each_swap(s,d2,i,j,all_scores);
...
int main(int argc, char **argv)
{
    ...
    if (is_there_something_to_score_it != multimap_scores.end()) {
      ...
      for (int i = 0; i < is_there_something_to_score_it->second.size(); i++) {
        ...
        score_each_swap(s,d,i,j,get_initial_score);
        ...
        enumerate_pairs(s, d, all_scores);
