    ...
    while (it != djs_sizes_descending_order.end()) {
       ...
       find_num2 = find(set.begin(), set.end(), num2);
        ...
        if ((find_num1 == set.end()) && (find_num2 == set.end()) && (d.Get_Elements()->at(d.Find(num1)).size() < 2) && (d.Get_Elements()->at(d.Find(num2)).size() < 2)) {
            ...
            // this means one of the elements of this set is in the scoring cells...
            for (eit = d.Get_Elements()->at(it->second).begin(); eit != d.Get_Elements()->at(it->second).end(); eit++) {
                if (s->goals[*eit]) {
                    // cout << "*eit " << *eit << endl;
                    in_scoring_cell = 1;
                    break; 
            ...
            set_size = d.Get_Elements()->at(it->second).size();
            ///////////////
        	...
        	if (s->board[num1] == s->board[it->second]) {
                ...
                // cout << "1/new_distance " << 1/new_distance << endl; 
            	if (1.0/new_distance > 1.0/original_distance) { 
            		swap_pair.push_back(num1);
            		swap_pair.push_back(num2);
                    swap_pair.push_back(in_scoring_cell);
                    swap_pair.push_back(set_size);
        	...
        	if (s->board[num2] == s->board[it->second]) {
                ...
                // cout << "new_distance " << new_distance << endl; 
                // cout << "1/new_distance " << 1.0/new_distance << endl; 
            	if (1.0/new_distance > 1.0/original_distance) { 
            		swap_pair.push_back(num1);
            		swap_pair.push_back(num2);
                    swap_pair.push_back(in_scoring_cell);
...
int main(int argc, char **argv)
{
    ...
    if (is_there_something_to_score_it != multimap_scores.end() && s->empty < 6) {
        ...
        for (size_t i = 0; i < all_scores_it->second.size(); i++) {
            ...
            multimap<double, vector <int>, greater<double> > updated_optimal_distances; 
            multimap<double, vector <int>, greater<double> >::iterator updated_optimal_distances_it;
            ...
            //     cout << "distances " << scores_min_distances_it->first << endl; 
            //      for (size_t i = 0; i < scores_min_distances_it->second.size(); i++) {
            //         cout << " " << scores_min_distances_it->second.at(i) << endl; 
            //     }
            // }

            scores_min_distances_it = scores_min_distances.begin();
            double least_distances = scores_min_distances_it->first;
            // cout << "least_distances " << least_distances << endl; 
            // try with only one distance to see if it works...
            while (scores_min_distances_it->first == least_distances && scores_min_distances_it != scores_min_distances.end()) {
                double points;
                vector <int> swap_pair;
                swap_pair.push_back(scores_min_distances_it->second.at(0));
                swap_pair.push_back(scores_min_distances_it->second.at(1));
                points = scores_min_distances_it->first * scores_min_distances_it->second.at(3);
                if (scores_min_distances_it->second.at(2)) {
                    points = scores_min_distances_it->first * 100;
                }   
                updated_optimal_distances.insert(make_pair(points, swap_pair));  

                scores_min_distances_it++;
            }

            // cout << "updated_optimal_distances_it " << endl;
            // for (updated_optimal_distances_it = updated_optimal_distances.begin(); updated_optimal_distances_it != updated_optimal_distances.end(); updated_optimal_distances_it++) {
            //     cout << "distances " << updated_optimal_distances_it->first << endl; 
            //     for (size_t i = 0; i < updated_optimal_distances_it->second.size(); i++) {
            //         cout << " " << updated_optimal_distances_it->second.at(i) << endl; 
            //     }
            // }

            updated_optimal_distances_it = updated_optimal_distances.begin();
            if (updated_optimal_distances_it != updated_optimal_distances.end()) {
                // cout << "swap in min distance " << endl; 
                cout << "SWAP";
                for (size_t i = 0; i < 2; i++) {
                    cout << " " << updated_optimal_distances_it->second.at(i)/s->c << " " << updated_optimal_distances_it->second.at(i)%s->c; 
                }
            }
                ...
                if (none_min_distance_it != none_min_distance.end()) {
                    // cout << "swap in none distance " << endl; 
                    cout << "SWAP";
